...
public record CreateTaskDto(String title, String description, ServiceStatus status, ServicePriority priority, LocalDateTime dueDate, LocalDateTime completedDate, Long clientId, String assignedToUserId, Long organizationId) {

    public static CreateTaskDto from(Task task){
        return new CreateTaskDto(task.getTitle(), task.getDescription(), task.getStatus(), task.getPriority(), task.getDueDate(), task.getCompletedDate(), task.getClient().getId(), task.getAssignedToUserId().getUsername(),  task.getOrganization().getId());
